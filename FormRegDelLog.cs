...
{
    public partial class FormRegDelLog : Form
    {
        private MySqlDB m_mysql;
        
        public FormRegDelLog()
        {
            InitializeComponent();
        }
        ...
        private void FormRegDelLog_Load(object sender, EventArgs e)
        {
            ...
            Program.CreateRoundRectRgn((Control)cb_lb_type);
            Program.CreateRoundRectRgn((Control)lv_log);
            //Program.CreateRoundRectRgn((Control)btn_search);
            //Program.CreateRoundRectRgn((Control)btn_close); 
        ...
        private void btn_search_Click(object sender, EventArgs e)
        {
            ...
            string _strType = "", _strDong = "", _strHo = "";
            
            cb_lb_name.InvokeIfNeeded(() => _strLBName = cb_lb_name.Text);
            cb_lb_name.InvokeIfNeeded(() => _strType = cb_lb_type.Text);

            if (_strType == "등록")
            {
                _strQry = string.Format("SELECT LogDate, Type, LobbyName, Dong, Ho, Comment FROM kms.Log_KmsKeyRegDel where LobbyName = '{0}' and Type = 'Reg' Order by Num desc limit 1000 ;", _strLBName);
            }
            else if (_strType == "삭제")
            {
                _strQry = string.Format("SELECT LogDate, Type, LobbyName, Dong, Ho, Comment FROM kms.Log_KmsKeyRegDel where LobbyName = '{0}' and Type = 'Del' Order by Num desc limit 1000 ;", _strLBName);
            }
            else // 전체
            {
                _strQry = string.Format("SELECT LogDate, Type, LobbyName, Dong, Ho, Comment FROM kms.Log_KmsKeyRegDel where LobbyName = '{0}' Order by Num desc limit 1000 ;", _strLBName);
            }
